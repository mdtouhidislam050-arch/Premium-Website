...
export const EditModal: React.FC<EditModalProps> = ({
  ...
  const [image, setImage] = useState<string>(''); // Base64 Data URL or URL
  ...
  useEffect(() => {
    if (app) {
      setName(app.name || '');
      setImage(app.image || app.imageUrl || '');
      setCategory(app.category || 'Apps');
      setDescription(app.description || '');
      setVersion(app.version || '1.0.0');
      setDeveloper(app.developer || '');
      setLink(app.link || app.downloadUrl || '');
      setNewFileName('');
      setNewFileSize('');
      setImageError(null);
      setErrors({});
  ...
  const handleProcessFile = async (file: File) => {
    ...
    try {
      ...
      setNewFileSize(formatFileSize(file.size));
      setImageError(null);
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800 bg-slate-900/90">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Update metadata, replace app image, or edit links for {app.name}
            </p>
          </div>
        ...
        <div className="px-6 py-2.5 bg-emerald-950/30 border-b border-emerald-500/20 flex items-center gap-2.5 text-xs text-emerald-300">
          ...
          <span>Updates are saved directly to this browser's local storage.</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                } text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 transition-all`}
              />
              {errors.name && (
          ...
          {/* Row 2: App Image (Keep existing or choose new file from computer) */}
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex flex-col sm:flex-row items-center gap-4">
                    {/* Trigger Button to replace image */}
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                      <button
                        id="edit-choose-image-btn"
                        type="button"
                      ...
                      >
                        {isConvertingImage ? (
                          ...
                          <>
                            ...
                            <span>{image ? 'Replace Image' : 'Choose App Image'}</span>
                          </>
                        )}
                      </button>
                    ...
                    {image && (
                      <div className="flex-1 w-full flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800">
                        <div className="text-right min-w-0">
                          <p className="text-xs font-semibold text-emerald-300 truncate max-w-[160px] sm:max-w-[200px]" title={newFileName || 'Current Image'}>
                            {newFileName || 'Current App Icon'}
                          </p>
                          <span className="text-[10px] text-slate-400 block">
                            {newFileSize ? `${newFileSize} (New)` : 'Saved in storage'}
                          </span>
                        </div>
          ...
          {/* Row 3: Download / Visit Link */}
          <div>
            <label 
              htmlFor="edit-app-download-link" 
              className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5"
            >
            ...
            <div className="relative">
              <input
                ...
                } text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 transition-all font-mono`}
              />
          ...
          {/* Row 4: Version & Developer */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label 
                htmlFor="edit-app-version" 
                className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5"
              >
                Version
              </label>
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-slate-800/80 border border-slate-700 text-white text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all"
              />
            </div>

            <div>
              <label 
                htmlFor="edit-app-developer" 
                className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5"
              >
                Developer Name
              </label>
              <input
                id="edit-app-developer"
                type="text"
                value={developer}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-800/80 border border-slate-700 text-white text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all resize-none"
            />
          </div>
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-end gap-3">
            ...
            <button
              ...
              type="submit"
              disabled={isConvertingImage}
              className="px-6 py-2.5 rounded-xl text-sm font-bold text-slate-950 bg-emerald-400 hover:bg-emerald-300 transition-all flex items-center gap-2 cursor-pointer shadow-md shadow-emerald-500/20 disabled:opacity-50"
            >
